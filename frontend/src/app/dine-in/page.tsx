...
export default function DineInPage() {
  ...
  useEffect(() => {
    if (searchQuery.trim() === '') {
      setFilteredMenus(dineInMenus)
    } else {
      const filtered = dineInMenus.filter((menu) =>
        menu.name.toLowerCase().includes(searchQuery.toLowerCase()),
      )
      setFilteredMenus(filtered)
    }
  }, [dineInMenus, searchQuery])
  ...
  const loadDineInMenus = async () => {
    try {
      const menusData = await apiService.getDineInMenus()
      setDineInMenus(menusData)
    } catch (error) {
      console.error('Failed to load dine-in menus:', error)
      showSnackbar('Không thể tải danh sách món ăn tại chỗ', 'error')
    }
  }

  const loadDineInOrders = async () => {
    try {
      const ordersData = await apiService.getTodayDineInOrders()
  ...
  const handleCreateOrder = async () => {
    ...
    try {
      ...
      await apiService.createDineInOrder(orderData)
  ...
  const updateOrderStatus = async (orderId: number, newStatus: TakeawayOrder['status']) => {
    try {
      await apiService.updateDineInOrderStatus(orderId, newStatus)
  ...
  const deleteOrder = async (orderId: number) => {
    ...
    try {
      await apiService.deleteDineInOrder(orderId)
      showSnackbar('Xóa đơn hàng thành công!')
      await Promise.all([loadDineInOrders(), loadDineInMenus()]) // Reload menus to update quantities
    } catch (error) {
  ...
  const handlePrintDineInInvoice = async () => {
  ...
  const getCategoryChip = (category: string) => {
    const getCategoryInfo = (cat: string) => {
      switch (cat) {
        case 'food':
          return { icon: <RestaurantIcon />, label: 'Đồ ăn', color: 'primary' as const }
        case 'drink':
          return { icon: <DinnerIcon />, label: 'Đồ uống', color: 'secondary' as const }
        case 'tobacco':
          return { icon: <RestaurantIcon />, label: 'Thuốc lá', color: 'warning' as const }
        default:
          return { icon: <RestaurantIcon />, label: 'Khác', color: 'default' as const }
      }
    }
  ...
  return (
    <Box sx={{ minHeight: '100vh', bgcolor: 'grey.50' }}>
      <AppBar title="Đặt đồ ăn tại chỗ" user={user} onLogout={handleLogout} />
                          ...
                          <ListItem sx={{ px: 0 }}>
                            ...
                            <Box>
                              ...
                              <IconButton
                                size="small"
                                onClick={() =>
                                  addToCart(dineInMenus.find((m) => m.id === item.menu_id)!)
                                }
                              >
                                <AddIcon />
                              </IconButton>
      ...
      <Dialog open={openInvoiceDialog} onClose={handleCloseInvoiceDialog} maxWidth="md" fullWidth>
        <DialogTitle>Hóa đơn tại chỗ - Đơn #{selectedOrder?.id}</DialogTitle>
        <DialogContent>
          {selectedOrder && (
            <Box sx={{ mt: 2 }}>
              <Typography variant="h6" gutterBottom>
        ...
        <DialogActions>
          ...
          <Button onClick={handlePrintDineInInvoice} variant="contained" color="primary">
            In hóa đơn
          </Button>
        </DialogActions>
      </Dialog>
